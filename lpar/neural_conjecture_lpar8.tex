...
\and
AI4REASON
\and University of Gothenburg / Chalmers University
}
...
Candidate lemmas are accepted when an ATP
proves both sides of the cut, i.e.\ the problem extended with the
lemma and with its negation.  Together with adjustable criteria for proof-search speedup this gives a
supervised learning signal and a learning-proving feedback loop.  We train and evaluate the methods on ATP datasets built from the \Mizar Mathematical
Library~\cite{Grabowski2010} with both \Eprover and
\Vampire .
%the \Vampire dataset contains 196,199 useful cut examples
%from 2.47M evaluated proof-lemma pairs.
On the \Eprover test set,
...
$196\times$.  We also show that a cut generator trained on the \Eprover data transfers
to \Vampire: when using AVATAR \emph{claims} as cuts \Vampire proves
% 582 previously unsolved \Mizar problems, including
501 hard \Mizar problems unsolved by Vampire or E before, and speeds up 2,236 further
problems. In combination with trained Deepire and Vampire's CASC mode, the learning/conjecturing/portfolio methods discussed here prove 1228 \Mizar problems unsolved by Vampire or E before.\footnote{The development of the neural architectures and the paper writing has used Claude Code and Codex in multiple human-supervised sessions. The experiments and evaluations were conducted by humans.}
\end{abstract}
...
libraries. This has been followed by many neural conjecturing systems done in various ITP contexts.

In the context of saturation-based refutational theorem proving, conjecturing takes
...
combine many sophisticated search heuristics, yet a wrong
trajectory may lead to millions of inferences before finding a short
...
neural generator on proof lemmas that have been measured
to shorten proof search, then use the ATP to evaluate the generated candidates.  The approach is attractive
...
The generated conjectures could also be used as proof-search
hints~\cite{Veroff1996Hints}.

In more detail, our pipeline is as follows:
\begin{enumerate}\itemsep0pt
\item Solve many related library problems and extract proof lemmas;
...
\item Use them to split the ATP problems, or as AVATAR \emph{claim} formulas.
\end{enumerate}
The code is developed online at our public repository.\footnote{\url{https://github.com/JUrban/E_conj_dev1/tree/audit-fixes6}}

% , trained checkpoints,
% generated conjectures, ATP command lines, logs, and scripts to recreate
% the tables are available at: \ArtifactURL.

Our main contributions are as follows.  We develop, train and evaluate with E several
symbol-independent graph-to-clause architectures for first-order cut
formula generation.
% Second, we construct much larger cut datasets than
% previous work, including 196K useful \Vampire examples.  Third, we give
% a prover-level evaluation rather than a purely syntactic or likelihood metric.
We then train Deepire on the relevant Mizar problems and prove without conjecturing 620 hard problems previously unsloved by E and Vampire.
Finally, we generate conjectures in two iterations of training and evaluation
and show that when added as AVATAR claims they help \Vampire solve hundreds of additional
hard \Mizar problems.

\section{Learning Cuts from Proofs}
\label{sec:cuts}

We use the framework for measuring lemma usefulness as ATP proof shortening introduced by Goertzel and Urban~\cite{Urban2019}.
In particular, they created a dataset of 3,161 \Mizar CNF problems with cut ratios which we start with here.
%and explored GNN-based lemma classification.
...
\]
A pair is useful when both subproofs succeed and $r<1$.  The cost is measured as
...
humans, but by measuring ATP search reduction. % in a sound proof experiment.
...
Unlike a standard transformer (or more generally, a language model), the generator does not emit symbol names from a fixed vocabulary.
...
role, and occurrence structure rather than on names alone.  This general \emph{anonymous} setting has been introduced in ATP by
\cite{OlsakKaliszykUrban2020} and used in various later systems such as \cite{PiepenbrockUrbanKorovinOlsakHeskesJanota2025}.
Optional
named \Mizar embeddings are here added for recurring symbols, but unknown
symbols still remain generatable through their graph embeddings. 

\paragraph{Decoder and syntax constraints:}
The decoder generates a clause as actions:
...
responsible for achieving high syntactic validity in the experiments, compared to earlier simpler unconstrained generation.

\paragraph{Training and variants:}
Our loss is the sum of cross-entropies for action, symbol pointer and
...
dominating training.  We experiment with training Transformer, conditional-VAE and
...
and mixed precision keep the GPUs\footnote{Most of the final experiments use the Leonardo HPC cluster with A100 GPUs.} saturated enough for experiments
...
problems.  All configurations use named \Mizar symbol embeddings.
$d$: hidden dimension.  Sampling: $T$ is temperature, $k$/p are
top-$k$/top-$p$ nucleus parameters.  AvgR is the best cut ratio
averaged over helped problems; lower is better.}
\label{tab:e}
\small
\begin{tabular}{lccrrrr}
\toprule
Decoder & $d$ & Sampling & Helped & \% & AvgR \\
\midrule
Transformer & 256  & $T$=1.5, $k$=25, p=.98 (wide)    & 96 & 51.3 & 0.558 \\
Transformer & 256  & $T$=1.2, $k$=15, p=.95 (diverse)  & 91 & 48.7 & 0.574 \\
Transformer & 256  & $T$=1.2, $k$=15, p=.95 (balanced)  & 87 & 46.5 & 0.524 \\
Transformer & 512  & $T$=1.2, $k$=15, p=.95 (diverse)  & 80 & 42.8 & 0.536 \\
CVAE        & 256  & $T$=1.2, $k$=15, p=.95             & 78 & 41.7 & 0.597 \\
SSM         & 256  & $T$=1.2, $k$=15, p=.95             & 72 & 38.5 & 0.612 \\
Transformer & 1024 & $T$=1.2, $k$=15, p=.95 (diverse)  & 67 & 35.8 & 0.530 \\
\midrule
\multicolumn{3}{l}{Union of all 15 configurations} & 127 & 67.9 & 0.530 \\
\bottomrule
\end{tabular}
\end{table}
...
model with named symbol embeddings and generate at most 50 conjectures for each
of 11,881 hard or previously unsolved Mizar60 problems, producing
517,435 conjectures with 98.9\% syntactic validity.  The top 42 conjectures
...
iterative prove-learn feedback loop.  Training from scratch on these examples
...
conjectures (at most 20 per problem) for the same 11,881 problems with
99.3\% syntactic validity.

\Vampire evaluation of the second iteration (Table~\ref{tab:iter2})
shows continued improvement.  With 20 conjectures from a model
trained on its own verified outputs, the second iteration adds 59
new hard problems beyond the first, bringing the cumulative total
to \textbf{1,121}---an 81\% increase over the baseline of 620.
Figure~\ref{fig:iter2rank} shows the number of hard problems solved
per conjecture rank: even rank~20 still solves 356 hard problems,
confirming that diverse sampling contributes throughout.
...
\begin{axis}[
    ...
    width=\columnwidth,
    height=4.5cm,
    xlabel={Conjecture rank},
    ylabel={Hard problems solved},
    ymin=300, ymax=600,
...
\addplot[fill=blue!60] coordinates {
    (1,555) (2,558) (3,544) (4,553) (5,536)
    (6,547) (7,518) (8,540) (9,522) (10,536)
    (11,532) (12,521) (13,490) (14,514) (15,506)
    (16,504) (17,483) (18,468) (19,442) (20,356)
};
\end{axis}
\end{tikzpicture}
\caption{Hard (minsub) problems solved per conjecture rank in the
  second iteration.  The baseline (no conjectures) solves 620.
  Each rank independently solves 356--558 hard problems; useful
  conjectures appear throughout, with a gradual decline for
  lower-ranked candidates.}
\label{fig:iter2rank}
\end{figure}

For comparison, \Vampire's CASC portfolio mode (30-second limit,
multiple strategies with AVATAR) solves 3,262 problems on the same
benchmark, of which only 289 are hard---less than half the 620
solved by the single trained Deepire~2.0 strategy.  Adding CASC to the
conjecturing results is complementary: the full union reaches
4,507 problems solved and \textbf{1,228 hard problems}, roughly
doubling the baseline.

\begin{table}[t]
\centering
\caption{Cumulative results on 11,881 hard/unsolved Mizar60 problems.
  ``Hard'' = never solved by any ATP in our comparison.}
\label{tab:iter2}
\small
\begin{tabular}{lrr}
\toprule
Configuration & Total & Hard \\
\midrule
\Vampire CASC portfolio (30s)          & 3,262 & 289 \\
Deepire AVATAR+neural (no conj.)      & 3,738 & 620 \\
\midrule
+ 1st iter.\ conj.\ (42, E-only)      & 4,320 & 1,062 \\
+ 2nd iter.\ conj.\ (20, combined)     & 4,389 & 1,121 \\
+ CASC portfolio                        & \textbf{4,507} & \textbf{1,228} \\
\bottomrule
\end{tabular}
\end{table}
...
some low-probability samples create large speedups.  Second,
...
clause guidance~\cite{KaliszykUrban2015FEMaLeCoP,JakubuvUrban2017ENIGMA,Loos2017,Jakubuv2019,Suda2021,RawsonReger2021lazyCoP,Chvalovsky2023}.
...
symbol-pointer graph generator in the spirit of \cite{PiepenbrockUrbanKorovinOlsakHeskesJanota2025}, arity-constrained decoding, large-scale
...
% 582
501 additional hard \Mizar problems (when run as claims under a trained neural Avatar),
% a second iteration using
% combined training data brings the total of newly proved hard
% problems to 492,
nearly doubling the baseline of 620 hard problems
solved by the trained neural \Vampire strategy.  Combined with \Vampire's CASC portfolio, the full union
reaches 1,228 hard problems.  These results suggest that learned cut formulas capture
...
\bibitem{KaliszykUrban2015FEMaLeCoP}
...
(LPAR-20 2015)},
volume 9450 of {\em Lecture Notes in Computer Science},
pages 88--96. Springer, 2015.
...
\bibitem{JakubuvUrban2017ENIGMA}
J.~Jakub\r{u}v and J.~Urban.
\newblock ENIGMA: Efficient learning-based inference guiding machine.
\newblock In H.~Geuvers, M.~England, O.~Hasan, F.~Rabe, and O.~Teschke, editors,
{\em Intelligent Computer Mathematics (CICM 2017)},
Lecture Notes in Computer Science, vol.~10383,
pages 292--302. Springer, 2017.
...
\newblock In \emph{ICLR 2020}, 2020.
...
\bibitem{RawsonReger2021lazyCoP}
M.~Rawson and G.~Reger.
\newblock lazyCoP: Lazy paramodulation meets neurally guided search.
\newblock In A.~Das and S.~Negri, editors,
{\em Automated Reasoning with Analytic Tableaux and Related Methods
(TABLEAUX 2021)},
Lecture Notes in Computer Science, vol.~12842,
pages 187--199. Springer, 2021.
...
\bibitem{OlsakKaliszykUrban2020}
M.~Ol\v{s}ák, C.~Kaliszyk, and J.~Urban.
\newblock Property invariant embedding for automated reasoning.
\newblock In G.~De Giacomo, A.~Catal\'a, B.~Dilkina, M.~Milano,
S.~Barro, A.~Bugar\'in, and J.~Lang, editors,
{\em ECAI 2020 -- 24th European Conference on Artificial Intelligence},
volume 325 of {\em Frontiers in Artificial Intelligence and Applications},
pages 1395--1402. IOS Press, 2020.

\bibitem{PiepenbrockUrbanKorovinOlsakHeskesJanota2025}
J.~Piepenbrock, J.~Urban, K.~Korovin, M.~Ol\v{s}\'ak,
T.~Heskes, and M.~Janota.
\newblock Invariant neural architecture for learning term synthesis in
instantiation proving.
\newblock {\em Journal of Symbolic Computation}, 128:102375, 2025.

\bibitem{Veroff1996Hints}
R.~Veroff.
\newblock Using hints to increase the effectiveness of an automated reasoning
program: Case studies.
\newblock {\em Journal of Automated Reasoning}, 16(3):223--239, 1996.
