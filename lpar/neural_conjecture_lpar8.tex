...
\title{Neural Cut Formula Generation for Automated Theorem Proving}

\author{Josef Urban\inst{1}}

\institute{Czech Technical University in Prague}

\authorrunning{Urban}
\titlerunning{Neural Cut Formula Generation for ATP}

\begin{document}
\maketitle

\begin{abstract}
We describe a tool for generating intermediate lemmas, used as cut
formulas, that restructure first-order proof search.  Given a CNF
problem, a heterogeneous graph neural network encodes clauses,
literals, terms, variables and symbols; an autoregressive decoder then
samples candidate lemmas by pointing to symbols in the input and by
obeying arity constraints during decoding.  Candidate lemmas are not
trusted: they are accepted only when an ATP proves both sides of the
cut, i.e. the problem extended with the lemma and with its negation.
This gives a sound evaluation loop and a supervised learning signal.
From the \Mizar Mathematical Library~\cite{Grabowski2010} we build datasets with both
\Eprover and \Vampire; the \Vampire dataset contains 196,199 useful
cut examples from 2.47M evaluated proof-lemma pairs.  On the \Eprover
test set, the best single configuration helps 96 of 187 baseline-proved
...
Large-theory automated theorem proving is still dominated by search.
Modern provers such as \Eprover~\cite{Schulz2002} and
\Vampire~\cite{Kovacs2013} combine many sophisticated heuristics, yet a
single unfortunate proof-search trajectory may spend millions of
inferences before finding a short refutation.  The classical cut rule
suggests a different intervention: introduce a formula $L$ that splits
a hard problem $P$ into two easier subproblems, $P\cup\{L\}$ and
$P\cup\{\neg L\}$.  If both are unsatisfiable, then $P$ is
unsatisfiable.  The formula $L$ need not be a user-interesting theorem;
it only has to expose a useful case split.

This paper treats cut discovery as a tool-building problem.  We train a
...
scheduler for generated candidates.  The resulting system is attractive
...
\end{enumerate}
The artifact for reviewers will contain code, trained checkpoints,
generated conjectures, ATP command lines, logs, and scripts to recreate
the tables: \ArtifactURL.
...
(problem, lemma) pairs.  The larger \Vampire data uses the full-axiom
corpus with neural clause selection~\cite{Suda2025}: \Vampire solves 34,182 of 57,880 attempted CNF problems, from
...
a premise-selected Mizar60 corpus of 55,832 problems, whose median CNF
...
state-space decoder variants inspired by selective state-space models~\cite{Gu2024}, varying dimension, named-symbol
...
\paragraph{Engineering for a tool.}
...
we use AVATAR claims with neural clause guidance~\cite{Suda2025} and a 100,000M
instruction limit plus a 30-second wall-clock limit.
...
\quad of which never solved by any ATP in our comparison & 442 \\
Total proved with baseline plus conjectures & 4,320 \\
...
11,881 problems.  The generated conjectures prove 582 additional
problems, a 15.6\% relative increase over the baseline.  The effect is
concentrated on the hardest part of the benchmark: 442 of the newly
proved problems are from the set not solved by any ATP configuration in
our comparison.  Among already solved problems, 2,236 obtain at least a
10\% search reduction; the mean ratio for the helped solved subset is
0.610, and the largest observed improvement exceeds $118\times$.
...
We used them to test whether conjecture generation can become an
...
\Eprover cuts with the new \Vampire cuts.  The resulting model generates
235,089 second-iteration conjectures for the same 11,881 problems with
99.3\% syntactic validity.  We do not count this as a main result until
its full prover evaluation is complete, but the experiment shows that
the tool already produces reusable training data from its own verified
successes.
...
can be scheduled greedily, but the current tool still relies on ATP
...
We presented a neural tool for generating cut formulas for automated
theorem proving.  The generated formulas are speculative but soundly
verified: a lemma counts only if the ATP proves both sides of the cut.
On \Eprover, ensembles help 67.9\% of baseline-proved test problems and
produce speedups up to $196\times$.  On \Vampire, conjectures learned
from \Eprover proof lemmas solve 582 additional hard \Mizar problems
and speed up 2,236 more.  These results suggest that learned cut
formulas capture reusable structure in mathematical proofs and can
serve as an effective interface between neural generation and symbolic
proof search.
...
\bibitem{Suda2025}
M. Suda.
\newblock Efficient neural clause-selection for Vampire.
...
\newblock In \emph{LPAR-24}, EPiC vol. 94, 2023.

\bibitem{Hu2020}
W. Hu, B. Fey, M. Zitnik, Y. Dong, H. Ren, B. Liu, M. Catasta, and J. Leskovec.
...
\newblock \emph{arXiv:2312.00752}, 2024.
